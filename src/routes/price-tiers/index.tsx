...
function RouteComponent() {
  ...
  const columns: ColumnDef<PriceTier>[] = [
    ...
    { header: "Deskripsi", accessorKey: "description" },
    {
      header: "Aksi",
      cell: ({ cell }) => (
        <DeletePriceTierDialog priceTier={cell.row.original} disabled={isReordering} />
      ),
    },
  ]
  ...
  return (
    <div className="p-6">
      ...
      <DataTable
        ...
        onReorder={isReordering ? (items) => setReorderData((items as PriceTier[]).map((item, idx) => ({ ...item, sortOrder: idx + 1 }))) : undefined}
      />
    </div>
  )
}
